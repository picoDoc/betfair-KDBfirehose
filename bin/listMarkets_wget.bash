...
function list_market_metadata() {
 ...
 OUT=`wget -qO- --header "Accept: application/json" --header "Content-Type: application/json" --header "X-Application: $APP_KEY" --header "X-Authentication:   $SESSION_TOKEN"  --post-data "{\"filter\":{},\"sort\":\"MAXIMUM_TRADED\",\"maxResults\":\"$MAX_RESULTS\",\"marketProjection\":[\"COMPETITION\",\"EVENT\",\"EVENT_TYPE\"]}" $HOST/rest/v1/listMarketCatalogue/`
 echo $OUT
}
